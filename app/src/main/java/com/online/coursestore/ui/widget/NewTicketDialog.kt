...
import com.online.coursestore.databinding.DialogNewTicketBinding
...
import com.online.coursestore.presenterImpl.NewTicketPresenterImpl
import java.io.File
...
class NewTicketDialog : NetworkObserverBottomSheetDialog(), View.OnClickListener {
    ...
    private var mOnTickeChatSaved: ItemCallback<TicketConversation>? = null
        ...
        when (v.id) {
            R.id.ticketDialogSendBtn -> {
                ...
                if (mTicketId == 0) {
                    ...
                    val conversation = TicketConversation()
    ...
    fun setOnTicketChatSavedListener(onTicketChatSaved: ItemCallback<TicketConversation>) {
        mOnTickeChatSaved = onTicketChatSaved
    }

    fun onTicketConversationSaved(conversation: TicketConversation, response: Data<Response>) {
        if (onSaved(response)) {
    ...
    fun onTicketConversationSaved(conversation: TicketConversation, response: BaseResponse) {
        if (onSaved(response)) {
            mOnTickeChatSaved?.onItem(conversation)
        }
    }
