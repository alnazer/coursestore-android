...
import com.online.coursestore.databinding.ItemFeaturedCourseBinding
...
import com.online.coursestore.model.PrerequisiteCourse
...
class FeaturedSliderAdapter<T>(items: List<T>, private val activity: MainActivity) :
    ...
    override fun onBindViewHolder(viewHolder: ViewHolder, position: Int) {
        ...
        if (course.rating > 0f){
            ...
            viewHolder.binding.itemFeaturedRatingBar.visibility = View.INVISIBLE
        }
    ...
    inner class ViewHolder(val binding: ItemFeaturedCourseBinding) :
        ...
        override fun onClick(v: View?) {
            val course: Course = when (val item = items[adapterPosition]) {
                is PrerequisiteCourse -> {
                    item.course
                }
                is Course -> {
